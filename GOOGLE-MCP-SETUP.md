...
Google verifies the staff identity; Leanne Digital issues the MCP access token after consent. Only gary@leannedigital.com and leanne@leannedigital.com can connect, including for reads. A Google-verified email is required. Google sign-in does not grant access to Gmail or Drive.
...
Complete a real Google sign-in, approve consent, then check a hosting total. An unapproved Google account must be refused. Credential updates require mcp:credentials:write and general CRUD requires mcp:write. Password sign-in and remote API-key access are disabled. See MCP-CRUD-DEPLOYMENT.md for reconnection and scope settings.
